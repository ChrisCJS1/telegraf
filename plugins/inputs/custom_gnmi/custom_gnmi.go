...
package custom_gnmi // RENAMED PACKAGE
...
// Regex to find a path segment that is a list predicate like "name[key=value]"
var multiPredicateSplitRegex = regexp.MustCompile(`\]\[`)

// parseFlexiblePath parses an XPath-like string into a gNMI Path,
// allowing '/' characters within list key identifiers.
// This function performs a more robust, iterative parsing of the path string.
func parseFlexiblePath(pathStr string) (*gnmi.Path, error) {
	gnmiPath := &gnmi.Path{}
	if pathStr == "" {
		return gnmiPath, nil
	}

	// Trim leading slash, if any, to simplify parsing
	pathStr = strings.TrimPrefix(pathStr, "/")
	if pathStr == "" {
		return gnmiPath, nil
	}

	remainingPath := pathStr
	for len(remainingPath) > 0 {
		elem := &gnmi.PathElem{}

		// Find the next '/' that is NOT inside a list predicate '[]'
		// Or find the start of a list predicate '['
		nextSlashIndex := -1
		nextBracketStartIndex := strings.Index(remainingPath, "[")

		// Find the first '/' that is not inside a bracket
		inBracket := false
		...
		if nextBracketStartIndex != -1 && (nextSlashIndex == -1 || nextBracketStartIndex < nextSlashIndex) {
			// Case: Current segment starts with an element name and then a list predicate.
			// Example: "monitored-object[meta/data/name/destination_name=mb32e]/metrics/delay-max"

			// Extract the element name before the bracket
			elem.Name = remainingPath[:nextBracketStartIndex]

			// Find the closing bracket for this predicate
			closingBracketIndex := strings.Index(remainingPath[nextBracketStartIndex:], "]")
			if closingBracketIndex == -1 {
				return nil, fmt.Errorf("malformed path: unclosed list predicate in '%s'", remainingPath)
			}

			// Extract the content inside the brackets, e.g., "meta/data/name/destination_name=mb32e"
			// The content is from nextBracketStartIndex+1 up to the closingBracketIndex relative to nextBracketStartIndex
			listPredicateContent := remainingPath[nextBracketStartIndex+1 : nextBracketStartIndex+closingBracketIndex]

			// Parse key-value pairs. Handle multiple predicates like [k1=v1][k2=v2]
			keys := make(map[string]string)
			// Split by "][" to handle multiple predicates within one element
			kvPairs := multiPredicateSplitRegex.Split(listPredicateContent, -1)
			for _, kv := range kvPairs {
				// Each kv should be "key=value"
				parts := strings.SplitN(kv, "=", 2) // Split only on the first '='
				if len(parts) != 2 {
					return nil, fmt.Errorf("malformed list predicate: expected key=value in '%s'", kv)
				}
				keys[parts[0]] = parts[1]
			}
			elem.Key = keys

			// Update remainingPath: skip the element name, the predicate, and the following '/' if present
			endOfCurrentElement := nextBracketStartIndex + closingBracketIndex + 1
			if endOfCurrentElement < len(remainingPath) && remainingPath[endOfCurrentElement] == '/' {
				remainingPath = remainingPath[endOfCurrentElement+1:] // Skip the '/' after the predicate
			} else {
				remainingPath = remainingPath[endOfCurrentElement:] // No '/' after, so this is the end or next segment starts immediately
			}

		} else if nextSlashIndex != -1 {
			// Case: Regular segment ending with a slash.
			// Example: "object-type/twamp-sf..."
			elem.Name = remainingPath[:nextSlashIndex]
			remainingPath = remainingPath[nextSlashIndex+1:]
		} else {
			// Case: Last segment in the path.
			// Example: ".../delay-max" when delay-max is the last part
			elem.Name = remainingPath
			remainingPath = "" // No more path remaining
		}

		// Only append if the element has a name or keys (avoid empty elements from double slashes etc.)
		if elem.Name != "" || elem.Key != nil {
...
type CustomGNMI struct { // RENAMED TYPE
	...
	PathParserMode string `toml:"path_parser_mode"`
...
func (*CustomGNMI) SampleConfig() string { // RENAMED RECEIVER
	return sampleConfig
}

func (c *CustomGNMI) Init() error { // RENAMED RECEIVER
	// Check options
	switch c.Encoding {
	case "":
		c.Encoding = "proto"
	...
	// Validate PathParserMode
	switch c.PathParserMode {
	case "", "xpath", "flexible":
	default:
		return fmt.Errorf("invalid 'path_parser_mode' %q", c.PathParserMode)
	}
	...
	for _, s := range c.Subscriptions {
		if err := s.buildAlias(c, c.internalAliases, c.EnforceFirstNamespaceAsOrigin); err != nil {
			return err
		}
	}
	...
	if _, err := c.ClientConfig.TLSConfig(); err != nil {
		if errors.Is(err, common_tls.ErrCipherUnsupported) {
			...
			for _, name := range insecure { // FIX: Removed duplicate 'name' declaration
				c.Log.Infof("  %s", name)
			}
		}
		return err
...
func (c *CustomGNMI) Start(acc telegraf.Accumulator) error { // RENAMED RECEIVER
	...
	for _, addr := range c.Addresses {
		go func(addr string) {
			...
			h := handler{
				...
				log:                           c.Log, // c.Log is still correct here
...
func (*CustomGNMI) Gather(telegraf.Accumulator) error { // RENAMED RECEIVER
	return nil
}

func (c *CustomGNMI) Stop() { // RENAMED RECEIVER
	c.cancel()
	c.wg.Wait()
}

func (s *subscription) buildSubscription(c *CustomGNMI) (*gnmi.Subscription, error) { // RENAMED RECEIVER
	gnmiPath, err := parsePath(c.PathParserMode, s.Origin, s.Path, "")
	if err != nil {
		return nil, err
	}
...
func (c *CustomGNMI) newSubscribeRequest() (*gnmi.SubscribeRequest, error) { // RENAMED RECEIVER
	...
	for _, subscription := range c.TagSubscriptions {
		sub, err := subscription.buildSubscription(c)
		if err != nil {
			return nil, err
		}
	...
	gnmiPath, err := parsePath(c.PathParserMode, c.Origin, c.Prefix, c.Target)
	if err != nil {
		return nil, err
	}
...
// parsePath acts as a dispatcher, choosing the appropriate parser based on parserMode.
// It takes parserMode as its first argument.
func parsePath(parserMode string, origin, pathToParse, target string) (*gnmi.Path, error) {
	var gnmiPath *gnmi.Path
	var err error

	if parserMode == "" {
		parserMode = "xpath" // Default to xpath if not explicitly set
	}
	...
	case "flexible":
		gnmiPath, err = parseFlexiblePath(pathToParse)
	default:
		// This case should ideally not be reached due to Init() validation
		return nil, fmt.Errorf("unknown path_parser_mode %q", parserMode)
	}
...
func (s *subscription) buildFullPath(c *CustomGNMI) error { // RENAMED RECEIVER
	var err error
	if s.fullPath, err = parsePath(c.PathParserMode, s.Origin, s.Path, ""); err != nil {
		return err
	}
	s.fullPath.Target = c.Target
	if c.Prefix != "" {
		prefix, err := parsePath(c.PathParserMode, c.Origin, c.Prefix, "")
		if err != nil {
			return err
		}
...
func (s *subscription) buildAlias(c *CustomGNMI, aliases map[*pathInfo]string, enforceFirstNamespaceAsOrigin bool) error { // RENAMED RECEIVER
	// Build the subscription path without keys
	path, err := parsePath(c.PathParserMode, s.Origin, s.Path, "")
	if err != nil {
		return err
	}
...
func init() {
	inputs.Add("custom_gnmi", func() telegraf.Input { // RENAMED PLUGIN NAME
		return &CustomGNMI{ // RENAMED TYPE
			Redial:                        config.Duration(10 * time.Second),
			EnforceFirstNamespaceAsOrigin: true,
			PathParserMode:                "xpath",
		}
	})
}
